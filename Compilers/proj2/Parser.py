...
class parser():
    ...
    def __init__(self, instring, debug_mode = False):
        ...
        self.LA = lexer(self.stack, instring)
        ...
        try: 
            ...
            if self.current_token[1] == target:
                if self.current_token[1] == "ID":
                    ...
                    self.debug('{:<10} {}'.format(str(target)+ " -> ", str(self.current_token[1]) ))
                self.current_token = next(self.pos)
            else: 
            ...
            print('Unexpected Exception: ',sys.exc_info()[0], 'Line: ', self.current_token)
            exit()
    '''
	Method: parse()
